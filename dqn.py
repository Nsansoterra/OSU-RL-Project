...
class DQN(nn.Module):
    def __init__(self, state_size, n_actions):
        ...
        # For vector input
        self.fc1 = NoisyLayer(state_size, 256)
        self.fc2 = NoisyLayer(256, 256)
        
        # Value stream
        self.fc1_value = NoisyLayer(256, 256)
        self.fc2_value = NoisyLayer(256, 1)
        
        # Advantage stream
        self.fc1_advantage = NoisyLayer(256, 256)
        self.fc2_advantage = NoisyLayer(256, n_actions)
...
policy_net = DQN(29, n_actions).to(device)
target_net = DQN(29, n_actions).to(device)
...
memory = ReplayMemory(10000, n_step=3, gamma=0.99)
...
for episode in count():
    ...
    if(episode % 100 == 0):
        torch.save(policy_net.state_dict(), f"dqn_model{episode}.pth")
    for t in count():
