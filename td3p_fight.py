...
def plot_avg_reward_agent2(show_result=False):
    plt.figure(5)
    plt.clf()
    plt.title('Agent 2 - Total Reward')
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    rewards_t = torch.tensor([float(r) for r in total_rewards2], dtype=torch.float)
...
def plot_goals_over_time(show_result=False):
    plt.figure(6)
    plt.clf()
    ...
    plt.plot(goal_history_blue, label='Blue Goals')
    plt.plot(goal_history_red, label='Red Goals')
...
def process_observation(timestep, player_idx):
    ...
    if player_idx == 0:
        ...
        enemy_dict = timestep.observation[0]
...
def get_observation_tensor(timestep, player_idx=0, device='cuda'):
    ...
    if player_idx == 0:
        ...
        obs_normalizer2.update(flat_obs[np.newaxis, :])  
        normalized_obs = obs_normalizer2.normalize(flat_obs)
...
actor_path = r"best_model\td3_actor_model_2000.pth"
critic_path = r"best_model\td3_critic_model_2000.pth"
actor_target_path = r"best_model\td3_actor_target_model_2000.pth"
critic_target_path = r"best_model\td3_critic_target_model_2000.pth"

agent.actor.load_state_dict(torch.load(actor_path, map_location=device))
agent.critic.load_state_dict(torch.load(critic_path, map_location=device))
agent.actor_target.load_state_dict(torch.load(actor_target_path, map_location=device))
...
agent2.actor.load_state_dict(torch.load(actor_path, map_location=device))
agent2.critic.load_state_dict(torch.load(critic_path, map_location=device))
agent2.actor_target.load_state_dict(torch.load(actor_target_path, map_location=device))
agent2.critic_target.load_state_dict(torch.load(critic_target_path, map_location=device))

obs_normalizer = RunningMeanStd(shape=(state_dim,))
obs_normalizer2 = RunningMeanStd(shape=(state_dim,))
with open("obs_normalizer.pkl", "rb") as f:
    obs_normalizer = pickle.load(f)
with open("obs_normalizer.pkl", "rb") as f:
    obs_normalizer2 = pickle.load(f)
memory = PrioritizedReplayBuffer(capacity=500000, alpha=PER_ALPHA, beta=PER_BETA, beta_increment=PER_BETA_INCREMENT)
memory2 = PrioritizedReplayBuffer(capacity=500000, alpha=PER_ALPHA, beta=PER_BETA, beta_increment=PER_BETA_INCREMENT)
...
ball_vel_history2 = deque(maxlen=12)  # Store last 10 velocities toward ball
goal_vel_history2 = deque(maxlen=12)  # Store last 10 velocities of ball toward goal
distance_history2 = deque(maxlen=12)  
...
for episode in count():
    ...
    distance_history2.clear()
    ball_vel_history2.clear()
    goal_vel_history2.clear()
    state, prev_ball_vel, prev_goal_vel = get_observation_tensor(timestep, player_idx=0, device=device)
    state2, prev_ball_vel2, prev_goal_vel2 = get_observation_tensor(timestep, player_idx=1, device=device)
    ...
    #if episode % 1000 == 0:
        #torch.save(agent.actor.state_dict(), f"td3_actor_model_vs_{episode}.pth")
       # torch.save(agent.critic.state_dict(), f"td3_critic_model_vs_{episode}.pth")
        #torch.save(agent.actor_target.state_dict(), f"td3_actor_target_model_vs_{episode}.pth")
        #torch.save(agent.critic_target.state_dict(), f"td3_critic_target_model_vs_{episode}.pth")
    for t in count():
        ...
        continuous_action2 = agent2.select_action(state2)
        actions.append(continuous_action)  
        actions.append(continuous_action2)


        if episode < 20 and episode%2 == 0:
            actions.clear()
            actions.append(np.random.uniform(0, 0, size=action_specs[0].shape))
            actions.append(continuous_action2)
        elif episode < 20:
            actions.clear()
            actions.append(continuous_action)
        ...
        for _ in range(FRAME_SKIP):
            ...
            distance_to_ball2 = np.linalg.norm(timestep.observation[1]['ball_ego_position'])
            ...
            new_state2, ball_vel2, goal_vel2 = get_observation_tensor(timestep, player_idx=1, device=device)
            if t > 10:
                goal_vel_history2.append(goal_vel2)
            distance_history2.append(distance_to_ball2)
            ...
            pixels2 = env.physics.render(camera_id=7, width=640, height=480)
            if steps_done % 4 == 0:
                image = cv2.cvtColor(pixels, cv2.COLOR_RGB2BGR)
                image2 = cv2.cvtColor(pixels2, cv2.COLOR_RGB2BGR)
                cv2.imshow('Soccer', image)        
                cv2.imshow('Soccer2', image2)
            ...
            if time_since_touch > 600 and time_since_touch2 > 600:
                done = True
                break
        ...
        if len(goal_vel_history2) >= 5:
            # Calculate if goal velocity is consistently improving
            goal_vel_trend = sum(goal_vel_history2[-1] - goal_vel_history2[i] for i in range(-5, -1)) / 4
            #if episode < 200:  # First phase - just hit the ball
            if goal_vel_trend > 0.05:
                time_since_touch = -100
                distance_history.clear()
        ...
        total_reward2 += frame_reward2
        reward = frame_reward
        reward2 = frame_reward2
        #~~~~~~~~~~~~~calculate reward~~~~~~~~~~~~~#
        ...
        action_tensor2 = torch.tensor(continuous_action2, dtype=torch.float32).unsqueeze(0).to(device)
        r2 = torch.tensor(reward2, dtype=torch.float, device=device)
        memory2.push((state2, action_tensor2, new_state2, r2, done))


        if episode > 25:

            loss12, loss22 = agent2.train(memory, BATCH_SIZE)
        ...
        if done:
            ...
            goal_history_blue.append(blue_goals)
            goal_history_red.append(red_goals)
            plot_avg_reward()
            plot_avg_reward_agent2()
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            torch.save(agent.actor.state_dict(), f"td31_actor_model_vs.pth")
            torch.save(agent.critic.state_dict(), f"td31_critic_model_vs.pth")
            torch.save(agent.actor_target.state_dict(), f"td31_actor_target_model_vs.pth")
            torch.save(agent.critic_target.state_dict(), f"td31_critic_target_model_vs.pth")
            torch.save(agent2.actor.state_dict(), f"td32_actor_model_vs.pth")
            torch.save(agent2.critic.state_dict(), f"td32_critic_model_vs.pth")
            torch.save(agent2.actor_target.state_dict(), f"td32_actor_target_model_vs.pth")
            torch.save(agent2.critic_target.state_dict(), f"td32_critic_target_model_vs.pth")
            with open("obs_normalizernew1.pkl", "wb") as f:
                pickle.dump(obs_normalizer, f)
            with open("obs_normalizernew2.pkl", "wb") as f:
                pickle.dump((obs_normalizer, obs_normalizer2), f) 
            exit(1)